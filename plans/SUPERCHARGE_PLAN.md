# PEARL Supercharge Plan: 6 Features

## Context

The goal is to add 5 features inspired by OpenClaw's architecture to the OECS Class Coworker (PEARL) app -- an offline-first Electron + React + Python educational AI tool running local GGUF LLMs on CPU (i7, 16GB RAM, no GPU). The features must add genuine value without slowing down the app. The primary model is Gemma 4.
...
| **Chat (Ask PEARL)** | Brain icon toggles `thinkingEnabled` (bool) in global settings | Brain icon opens popup with **Quick / Deep** (local to Chat, not in settings) |
...
mainWindow.on('hide', async () => {
  try {
    await fetch(`http://127.0.0.1:${BACKEND_PORT}/api/model/unload`, { method: 'POST' });
...
mainWindow.on('show', async () => {
  try {
    await fetch(`http://127.0.0.1:${BACKEND_PORT}/api/model/reload`, { method: 'POST' });
...
## Feature 6: PEARL Long-Term Memory (Cross-Conversation Recall)

### What It Is

Ask PEARL currently remembers within a single conversation (via the 3-tier summary system) but starts every new conversation blank. This feature adds a **teacher memory layer** that persists key facts across conversations, so PEARL recalls preferences, patterns, and context over time.
...
Teacher chats with PEARL (10+ messages)
...
Teacher sends message in a new (or existing) conversation
  ...
  -> PEARL's response is informed by long-term context
...
| 2 | Feature 6: PEARL Long-Term Memory | 1.5 days | Medium -- expanded summary prompt + TF-IDF search |
| 3 | Feature 2: Thinking Controls | 1 day | Low -- mostly frontend + prompt routing |
| 4 | Feature 5A: PlanningHub wrapper (merge 2 tabs + shell) | 0.5 day | Low -- wrapping existing components |
| 5 | Feature 5B: Timetable backend + WeeklyTimetable component | 2 days | Medium -- new CRUD + interactive grid UI |
| 6 | Feature 3A: Model Offloading | 1 day | Medium -- GC behavior, reload timing |
| 7 | Feature 4: Analyse Chat | 2-3 days | Medium -- new WS endpoint + panel enhancement |
| 8 | Feature 3B: Schedule Config | 1 day | Low -- APScheduler already exists |
| 9 | Feature 3C: ELO Breakdown Task (uses timetable) | 2 days | Medium -- LLM prompt engineering + review UI |
| 10 | Feature 3D: Attendance + Progress Tasks | 1 day | Low -- follow ELO pattern |
| 11 | Feature 3E: Notification Panel Update | 1 day | Low -- extend existing components |

**Rationale for order:** Features 1 & 6 improve memory together (context budgeting + long-term recall). Feature 2 (thinking controls) is a quick frontend win. Feature 5 (PlanningHub + Timetable) must come before ELO breakdown (3C) because timetable is a required input. Model offloading (3A) is a prerequisite for background scheduling. Feature 4 (Analyse) is the biggest UX change. Schedule infrastructure (3B-3E) layers on top last.
...
### Feature 1
- Open Ask PEARL, send 20+ messages in a conversation
...
### Feature 6
- Chat with PEARL about fractions for 10+ messages -> verify summary fires AND key_facts extracted
- Check SQLite `teacher_memories` table -> verify facts stored
- Start a NEW conversation -> mention "math" -> verify PEARL's response reflects recalled memory ("I remember you mentioned your Grade 3 class is behind on fractions")
