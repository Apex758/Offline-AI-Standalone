...
// Interactive literary devices component for Grade 4
function InteractiveLiteraryDevices() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setActiveDevices((prev) => [...prev.slice(-3), randomIndex])
    }, 1200)
  ...
  return (
    <div className="flex flex-wrap justify-center gap-2 sm:gap-3 lg:gap-2 xl:gap-3 p-3 sm:p-6 lg:p-6 xl:p-8 bg-gradient-to-r from-purple-100 via-blue-100 to-teal-100 rounded-xl sm:rounded-2xl lg:rounded-2xl">
      {devices.map((device, index) => (
        <div
          ...
          className={`px-3 py-2 sm:px-4 sm:py-3 lg:px-4 lg:py-3 xl:px-5 xl:py-3 rounded-full flex items-center justify-center text-xs sm:text-sm lg:text-sm xl:text-base font-bold cursor-pointer transition-all duration-300 ${
            activeDevices.includes(index)
              ? "bg-gradient-to-r from-purple-500 to-teal-500 text-white scale-110 sm:scale-125 lg:scale-130 shadow-lg"
              : "bg-white text-purple-600 hover:bg-purple-100 hover:scale-105 sm:hover:scale-110 lg:hover:scale-115"
          }`}
          onClick={() => setActiveDevices((prev) => [...prev, index])}
        >
          {device}
        </div>
      ))}
...
export default function Grade4LanguageArtsPage() {
  ...
  const strands = [
    {
      ...
      icon: Headphones,
      description: "Developing oral language skills for communication",
      content:
        "In Grade 4, students enhance their listening and speaking skills to communicate effectively in various contexts. They learn to listen attentively, respond critically, and express ideas with clarity and confidence. Students engage with various genres of music, oral poetry, and stories while developing an appreciation for both Standard English and Home Languages.",
      ...
      link: "/curriculum/grade4-subjects/language-arts/listening-speaking",
    },
    {
      id: 2,
      title: "Reading & Viewing",
      icon: Eye,
      description: "Developing comprehension and critical thinking",
      content:
        "Students build on their reading skills by engaging with a variety of texts at the transitional to fluent levels. They develop advanced comprehension strategies, vocabulary, and critical thinking skills while exploring different genres. Students learn to analyze how an author's choice of vocabulary, language, and style influence the meaning and impact of text.",
      ...
      link: "/curriculum/grade4-subjects/language-arts/reading-viewing",
    },
    {
      id: 3,
      title: "Writing & Representing",
      icon: PenTool,
      description: "Developing written expression and communication",
      content:
        "Grade 4 students master their writing skills through the complete writing process: planning, drafting, revising, editing, and publishing. They learn to write for various purposes and audiences, using different genres and text forms with increasing sophistication. Students develop advanced understanding of writing conventions and literary devices.",
      ...
      link: "/curriculum/grade4-subjects/language-arts/writing-representing",
    },
  ]
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-teal-50">
      ...
      <div className="hidden sm:block fixed inset-0 pointer-events-none overflow-hidden">
        <FloatingElement delay={0}>
          <BookOpen className="absolute top-20 left-10 text-purple-400 w-4 h-4 sm:w-6 sm:h-6" />
        </FloatingElement>
        <FloatingElement delay={1}>
          <Award className="absolute top-40 right-20 text-teal-400 w-6 h-6 sm:w-8 sm:h-8" />
        </FloatingElement>
      ...
      <div className="container mx-auto px-4 lg:px-6 xl:px-8 py-4 sm:py-8 lg:py-10 xl:py-12 relative z-10 max-w-7xl">
        <Link to="/curriculum/grade4-subjects">
          <Button
            variant="outline"
            className="mb-4 sm:mb-6 hover:scale-105 transition-transform bg-transparent text-sm sm:text-base"
          >
            <ChevronLeft className="mr-1 sm:mr-2 h-3 w-3 sm:h-4 sm:w-4" /> Back to Grade 4 Subjects
          </Button>
        </Link>

        {/* Hero Section */}
        <div className="relative mb-8 sm:mb-12 lg:mb-16">
          <div className="bg-gradient-to-r from-purple-400 via-blue-400 to-teal-400 p-4 sm:p-8 lg:p-10 xl:p-12 rounded-2xl sm:rounded-3xl shadow-xl sm:shadow-2xl transform hover:scale-105 transition-all duration-300">
            <div className="bg-white/90 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-7 xl:p-8">
              <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-4xl xl:text-5xl font-bold mb-3 sm:mb-6 lg:mb-6 text-center bg-clip-text text-transparent bg-gradient-to-r from-purple-600 via-blue-600 to-teal-600 animate-pulse">
              ...
              <p className="text-sm sm:text-lg md:text-xl lg:text-xl xl:text-2xl text-gray-700 text-center max-w-4xl lg:max-w-5xl xl:max-w-6xl mx-auto leading-relaxed">
                Language Arts provides essential skills for understanding, communicating, and expressing yourself!
                Master advanced reading, writing, and speaking through engaging hands-on experiences! 📚✨
              </p>
            </div>
          </div>
        </div>
                ...
                <CardContent className="flex-grow p-4 sm:p-6 lg:p-6 flex flex-col">
                  ...
                  <div className="bg-yellow-100 p-3 sm:p-4 lg:p-4 rounded-lg mb-3 sm:mb-4 lg:mb-4 border-l-4 border-yellow-400">
                    <p className="text-xs sm:text-sm lg:text-sm text-yellow-800 font-medium">
                      💡 Grade 4 Tip:{" "}
                      {strand.id === 1
                        ? "Practice debates and formal presentations!"
                        : strand.id === 2
                          ? "Analyze author's techniques and literary devices!"
                          : "Use advanced vocabulary and literary devices in writing!"}
                    </p>
                  </div>
        ...
        {/* Interactive Literary Devices Section */}
        <div className="mb-8 sm:mb-12 lg:mb-16">
          <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-3xl xl:text-4xl font-bold mb-4 sm:mb-6 lg:mb-6 text-center text-purple-700 flex items-center justify-center gap-1 sm:gap-2 lg:gap-2">
            <span>🎭</span> Interactive Literary Devices! <span>🎭</span>
          </h2>
          <div className="max-w-7xl mx-auto">
            <InteractiveLiteraryDevices />
          </div>
          <p className="text-center mt-3 sm:mt-4 lg:mt-4 text-gray-600 text-sm sm:text-base lg:text-base">
            Click on literary devices to see them light up! Master advanced language techniques! ✨
          </p>
        </div>
        ...
        <div className="bg-gradient-to-br from-indigo-100 via-purple-100 to-blue-100 p-4 sm:p-6 md:p-8 lg:p-8 xl:p-10 rounded-2xl sm:rounded-3xl shadow-lg sm:shadow-xl border border-indigo-200 sm:border-2 mb-8 sm:mb-12 lg:mb-12 relative overflow-hidden max-w-6xl mx-auto">
          ...
          <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-3xl xl:text-4xl font-bold mb-4 sm:mb-6 lg:mb-6 text-indigo-700">
            Grade 4 Language Arts Overview!
          </h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 xl:gap-12">
            <div className="lg:col-span-1 xl:col-span-2">
              <p className="mb-3 sm:mb-4 lg:mb-4 text-gray-700 text-sm sm:text-lg lg:text-base xl:text-lg leading-relaxed">
                The Grade 4 curriculum builds upon earlier foundations and encourages deeper understanding, critical
                thinking skills, and greater independence in language development. Students become confident
                communicators! 🎯
              </p>
              <p className="text-gray-700 text-sm sm:text-lg lg:text-base xl:text-lg leading-relaxed">
                The curriculum celebrates OECS culture and context, incorporating local stories, songs, and poetry while
                developing advanced literary analysis skills! 🌴
              </p>
            </div>
            ...
            <div className="bg-white/70 p-4 sm:p-6 lg:p-6 xl:p-7 rounded-xl sm:rounded-2xl lg:col-span-1">
              ...
              <ul className="space-y-1 sm:space-y-2 lg:space-y-3">
                <li className="flex items-center gap-2 lg:gap-2 text-sm sm:text-base lg:text-base xl:text-lg">
                  ...
                  <span>Advanced oral presentations</span>
                </li>
                <li className="flex items-center gap-2 lg:gap-2 text-sm sm:text-base lg:text-base xl:text-lg">
                  <span className="text-green-500 text-lg lg:text-lg">✓</span>
                  <span>Fluent reading levels</span>
                </li>
                <li className="flex items-center gap-2 lg:gap-2 text-sm sm:text-base lg:text-base xl:text-lg">
                  <span className="text-green-500 text-lg lg:text-lg">✓</span>
                  <span>Sophisticated writing techniques</span>
                </li>
                <li className="flex items-center gap-2 lg:gap-2 text-sm sm:text-base lg:text-base xl:text-lg">
                  <span className="text-green-500 text-lg lg:text-lg">✓</span>
                  <span>Literary device analysis</span>
                </li>
              </ul>
            </div>
          </div>
