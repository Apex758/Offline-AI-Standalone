...
// Interactive story elements component for Grade 3
function InteractiveStoryElements() {
  const [activeElements, setActiveElements] = useState<number[]>([])
  const elements = ["CHARACTER", "SETTING", "PLOT", "CONFLICT", "THEME", "DIALOGUE", "NARRATOR", "CLIMAX"]

  useEffect(() => {
    const interval = setInterval(() => {
      const randomIndex = Math.floor(Math.random() * elements.length)
      setActiveElements((prev) => [...prev.slice(-3), randomIndex])
    }, 1000)
  ...
  return (
    <div className="flex flex-wrap justify-center gap-2 sm:gap-3 lg:gap-2 xl:gap-3 p-3 sm:p-6 lg:p-6 xl:p-8 bg-gradient-to-r from-purple-100 via-blue-100 to-green-100 rounded-xl sm:rounded-2xl lg:rounded-2xl">
        ...
        <div
          ...
          className={`px-3 py-2 sm:px-4 sm:py-3 lg:px-4 lg:py-3 xl:px-5 xl:py-3 rounded-full flex items-center justify-center text-xs sm:text-sm lg:text-sm xl:text-base font-bold cursor-pointer transition-all duration-300 ${
            activeElements.includes(index)
              ? "bg-gradient-to-r from-purple-500 to-blue-500 text-white scale-110 sm:scale-125 lg:scale-130 shadow-lg"
              : "bg-white text-purple-600 hover:bg-purple-100 hover:scale-105 sm:hover:scale-110 lg:hover:scale-115"
          }`}
          onClick={() => setActiveElements((prev) => [...prev, index])}
        >
          {element}
        </div>
      ))}
  ...
  const strands = [
    {
      ...
      link: "/curriculum/grade3-subjects/language-arts/listening-speaking",
    },
    {
      id: 2,
      ...
      content:
        "Students build on their reading skills by engaging with a variety of texts at the early to transitional levels. They develop comprehension strategies, vocabulary, and critical thinking skills while exploring different genres. Students learn to recognize how an author's choice of vocabulary, language, and style influence the meaning of text.",
      ...
      link: "/curriculum/grade3-subjects/language-arts/reading-viewing",
    },
    {
      id: 3,
      ...
      content:
        "Grade 3 students develop their writing skills through the writing process: planning, drafting, revising, editing, and publishing. They learn to write for various purposes and audiences, using different genres and text forms. Students also develop their understanding of writing conventions, including spelling, punctuation, and presentation.",
      ...
      link: "/curriculum/grade3-subjects/language-arts/writing-representing",
    },
  ]
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-green-50">
      ...
      <div className="hidden sm:block fixed inset-0 pointer-events-none overflow-hidden">
        ...
        <FloatingElement delay={1}>
          <Trophy className="absolute top-40 right-20 text-green-400 w-6 h-6 sm:w-8 sm:h-8" />
        </FloatingElement>
        <FloatingElement delay={2}>
          <Zap className="absolute bottom-40 left-20 text-blue-400 w-5 h-5 sm:w-7 sm:h-7" />
        </FloatingElement>
        <FloatingElement delay={0.5}>
          <Star className="absolute bottom-20 right-10 text-purple-400 w-4 h-4 sm:w-5 sm:h-5" />
        </FloatingElement>
      </div>
      ...
      <div className="container mx-auto px-4 lg:px-6 xl:px-8 py-4 sm:py-8 lg:py-10 xl:py-12 relative z-10 max-w-7xl">
        <Link to="/curriculum/grade3-subjects">
          ...
          >
            <ChevronLeft className="mr-1 sm:mr-2 h-3 w-3 sm:h-4 sm:w-4" /> Back to Grade 3 Subjects
          </Button>
        </Link>

        {/* Hero Section */}
        <div className="relative mb-8 sm:mb-12 lg:mb-16">
          <div className="bg-gradient-to-r from-purple-400 via-blue-400 to-green-400 p-4 sm:p-8 lg:p-10 xl:p-12 rounded-2xl sm:rounded-3xl shadow-xl sm:shadow-2xl transform hover:scale-105 transition-all duration-300">
            <div className="bg-white/90 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-7 xl:p-8">
              <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-4xl xl:text-5xl font-bold mb-3 sm:mb-6 lg:mb-6 text-center bg-clip-text text-transparent bg-gradient-to-r from-purple-600 via-blue-600 to-green-600 animate-pulse">
                ...
                <CardContent className="flex-grow p-4 sm:p-6 lg:p-6 flex flex-col">
                  ...
                  <div className="bg-yellow-100 p-3 sm:p-4 lg:p-4 rounded-lg mb-3 sm:mb-4 lg:mb-4 border-l-4 border-yellow-400">
                    <p className="text-xs sm:text-sm lg:text-sm text-yellow-800 font-medium">
                      💡 Grade 3 Tip:{" "}
                      {strand.id === 1
                        ? "Practice presenting to your classmates!"
                        : strand.id === 2
                          ? "Look for author's style and word choices!"
        ...
        {/* Interactive Story Elements Section */}
        <div className="mb-8 sm:mb-12 lg:mb-16">
          <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-3xl xl:text-4xl font-bold mb-4 sm:mb-6 lg:mb-6 text-center text-purple-700 flex items-center justify-center gap-1 sm:gap-2 lg:gap-2">
            <span>📖</span> Interactive Story Elements! <span>📖</span>
          </h2>
          ...
          <p className="text-center mt-3 sm:mt-4 lg:mt-4 text-gray-600 text-sm sm:text-base lg:text-base">
            Click on story elements to see them light up! Master storytelling! ✨
          </p>
        </div>
        ...
        <div className="bg-gradient-to-br from-indigo-100 via-purple-100 to-blue-100 p-4 sm:p-6 md:p-8 lg:p-8 xl:p-10 rounded-2xl sm:rounded-3xl shadow-lg sm:shadow-xl border border-indigo-200 sm:border-2 mb-8 sm:mb-12 lg:mb-12 relative overflow-hidden max-w-6xl mx-auto">
          ...
          <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-3xl xl:text-4xl font-bold mb-4 sm:mb-6 lg:mb-6 text-indigo-700">
            Grade 3 Language Arts Overview!
          </h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 xl:gap-12">
            <div className="lg:col-span-1 xl:col-span-2">
              <p className="mb-3 sm:mb-4 lg:mb-4 text-gray-700 text-sm sm:text-lg lg:text-base xl:text-lg leading-relaxed">
                The Grade 3 curriculum builds upon earlier foundations and encourages deeper understanding and critical
                thinking skills. Students develop greater independence in their language development! 🎯
              </p>
              <p className="text-gray-700 text-sm sm:text-lg lg:text-base xl:text-lg leading-relaxed">
                The curriculum celebrates OECS culture and context, incorporating local stories, songs, and poetry
            ...
            <div className="bg-white/70 p-4 sm:p-6 lg:p-6 xl:p-7 rounded-xl sm:rounded-2xl lg:col-span-1">
              ...
              <ul className="space-y-1 sm:space-y-2 lg:space-y-3">
                <li className="flex items-center gap-2 lg:gap-2 text-sm sm:text-base lg:text-base xl:text-lg">
                  ...
                  <span>Critical listening & speaking</span>
                </li>
                <li className="flex items-center gap-2 lg:gap-2 text-sm sm:text-base lg:text-base xl:text-lg">
                  <span className="text-green-500 text-lg lg:text-lg">✓</span>
                  <span>Transitional reading levels</span>
                </li>
                <li className="flex items-center gap-2 lg:gap-2 text-sm sm:text-base lg:text-base xl:text-lg">
                  <span className="text-green-500 text-lg lg:text-lg">✓</span>
                  <span>Complete writing process</span>
                </li>
                <li className="flex items-center gap-2 lg:gap-2 text-sm sm:text-base lg:text-base xl:text-lg">
                  <span className="text-green-500 text-lg lg:text-lg">✓</span>
                  <span>Author's style analysis</span>
                </li>
              </ul>
            </div>
          </div>
