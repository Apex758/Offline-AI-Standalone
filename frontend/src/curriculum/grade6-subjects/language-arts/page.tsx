...
// Interactive critical thinking component for Grade 6
function InteractiveCriticalThinking() {
  const [activeSkills, setActiveSkills] = useState<number[]>([])
  const skills = ["EVALUATE", "SYNTHESIZE", "COMPARE", "CONTRAST", "JUSTIFY", "INTERPRET", "DEBATE", "RESEARCH"]

  useEffect(() => {
    const interval = setInterval(() => {
      const randomIndex = Math.floor(Math.random() * skills.length)
      setActiveSkills((prev) => [...prev.slice(-3), randomIndex])
    }, 1600)
  ...
  return (
    <div className="flex flex-wrap justify-center gap-2 sm:gap-3 lg:gap-2 xl:gap-3 p-3 sm:p-6 lg:p-6 xl:p-8 bg-gradient-to-r from-purple-100 via-blue-100 to-indigo-100 rounded-xl sm:rounded-2xl lg:rounded-2xl">
        ...
        <div
          ...
          className={`px-3 py-2 sm:px-4 sm:py-3 lg:px-4 lg:py-3 xl:px-5 xl:py-3 rounded-full flex items-center justify-center text-xs sm:text-sm lg:text-sm xl:text-base font-bold cursor-pointer transition-all duration-300 ${
            activeSkills.includes(index)
              ? "bg-gradient-to-r from-purple-500 to-indigo-500 text-white scale-110 sm:scale-125 lg:scale-130 shadow-lg"
              : "bg-white text-purple-600 hover:bg-purple-100 hover:scale-105 sm:hover:scale-110 lg:hover:scale-115"
          }`}
          onClick={() => setActiveSkills((prev) => [...prev, index])}
        >
          {skill}
        </div>
      ))}
  ...
  const strands = [
    {
      ...
      content:
        "Grade 6 students refine their listening and speaking skills to engage in critical discourse and express themselves effectively in diverse contexts. They learn to analyze persuasive techniques, adapt their language to different situations, and appreciate the nuances of both Standard English and Caribbean dialects.",
      ...
      link: "/curriculum/grade6-subjects/language-arts/listening-speaking",
    },
    {
      id: 2,
      ...
      link: "/curriculum/grade6-subjects/language-arts/reading-viewing",
    },
    {
      id: 3,
      ...
      content:
        "Grade 6 students master the complete writing process through sophisticated composition across multiple genres. They develop advanced revision and editing skills, explore complex sentence structures, and create polished publications for authentic audiences. Students integrate technology, visual elements, and multimedia to enhance their written communication while maintaining cultural authenticity.",
      ...
      link: "/curriculum/grade6-subjects/language-arts/writing-representing",
    },
  ]
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-50">
      ...
      <div className="hidden sm:block fixed inset-0 pointer-events-none overflow-hidden">
        <FloatingElement delay={0}>
          <Users className="absolute top-20 left-10 text-purple-400 w-4 h-4 sm:w-6 sm:h-6" />
        </FloatingElement>
        <FloatingElement delay={1}>
          <Award className="absolute top-40 right-20 text-indigo-400 w-6 h-6 sm:w-8 sm:h-8" />
        </FloatingElement>
        <FloatingElement delay={2}>
          <Globe className="absolute bottom-40 left-20 text-blue-400 w-5 h-5 sm:w-7 sm:h-7" />
        </FloatingElement>
      ...
      <div className="container mx-auto px-4 lg:px-6 xl:px-8 py-4 sm:py-8 lg:py-10 xl:py-12 relative z-10 max-w-7xl">
        <Link to="/curriculum/grade6-subjects">
          ...
          >
            <ChevronLeft className="mr-1 sm:mr-2 h-3 w-3 sm:h-4 sm:w-4" /> Back to Grade 6 Subjects
          </Button>
        </Link>

        {/* Hero Section */}
        <div className="relative mb-8 sm:mb-12 lg:mb-16">
          <div className="bg-gradient-to-r from-purple-400 via-blue-400 to-indigo-400 p-4 sm:p-8 lg:p-10 xl:p-12 rounded-2xl sm:rounded-3xl shadow-xl sm:shadow-2xl transform hover:scale-105 transition-all duration-300">
            <div className="bg-white/90 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-7 xl:p-8">
              <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-4xl xl:text-5xl font-bold mb-3 sm:mb-6 lg:mb-6 text-center bg-clip-text text-transparent bg-gradient-to-r from-purple-600 via-blue-600 to-indigo-600 animate-pulse">
              ...
              <p className="text-sm sm:text-lg md:text-xl lg:text-xl xl:text-2xl text-gray-700 text-center max-w-4xl lg:max-w-5xl xl:max-w-6xl mx-auto leading-relaxed">
                Building upon earlier foundations, prepare for advanced literacy practices and critical analysis! Engage
                in complex discussions and celebrate Caribbean culture while developing global perspectives! 📚✨
              </p>
            </div>
          </div>
        </div>
                ...
                <CardContent className="flex-grow p-4 sm:p-6 lg:p-6 flex flex-col">
                  ...
                  <div className="bg-yellow-100 p-3 sm:p-4 lg:p-4 rounded-lg mb-3 sm:mb-4 lg:mb-4 border-l-4 border-yellow-400">
                    <p className="text-xs sm:text-sm lg:text-sm text-yellow-800 font-medium">
                      💡 Grade 6 Excellence:{" "}
                      {strand.id === 1
                        ? "Master persuasive techniques and critical discourse!"
                        : strand.id === 2
                          ? "Analyze complex themes across cultures and texts!"
                          : "Create sophisticated digital publications!"}
                    </p>
                  </div>
        ...
        {/* Interactive Critical Thinking Section */}
        <div className="mb-8 sm:mb-12 lg:mb-16">
          <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-3xl xl:text-4xl font-bold mb-4 sm:mb-6 lg:mb-6 text-center text-purple-700 flex items-center justify-center gap-1 sm:gap-2 lg:gap-2">
            <span>🧠</span> Critical Thinking Skills! <span>🧠</span>
          </h2>
          <div className="max-w-7xl mx-auto">
            <InteractiveCriticalThinking />
          </div>
          <p className="text-center mt-3 sm:mt-4 lg:mt-4 text-gray-600 text-sm sm:text-base lg:text-base">
            Click on skills to see them light up! Master advanced critical thinking! ✨
          </p>
        </div>
        ...
        <div className="bg-gradient-to-br from-indigo-100 via-purple-100 to-blue-100 p-4 sm:p-6 md:p-8 lg:p-8 xl:p-10 rounded-2xl sm:rounded-3xl shadow-lg sm:shadow-xl border border-indigo-200 sm:border-2 mb-8 sm:mb-12 lg:mb-12 relative overflow-hidden max-w-6xl mx-auto">
          ...
          <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-3xl xl:text-4xl font-bold mb-4 sm:mb-6 lg:mb-6 text-indigo-700">
            Grade 6 Language Arts Overview!
          </h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 xl:gap-12">
            <div className="lg:col-span-1 xl:col-span-2">
              <p className="mb-3 sm:mb-4 lg:mb-4 text-gray-700 text-sm sm:text-lg lg:text-base xl:text-lg leading-relaxed">
                The Grade 6 curriculum builds upon earlier foundations, preparing students for advanced literacy
                practices and critical analysis. Students engage with authentic Caribbean contexts while developing
                global perspectives! 🎯
              </p>
              <p className="text-gray-700 text-sm sm:text-lg lg:text-base xl:text-lg leading-relaxed">
                The curriculum integrates all language arts strands through meaningful connections, supporting diverse
                learners while celebrating cultural heritage and preparing for lifelong learning! 🌴
              </p>
            </div>
            ...
            <div className="bg-white/70 p-4 sm:p-6 lg:p-6 xl:p-7 rounded-xl sm:rounded-2xl lg:col-span-1">
              ...
              <ul className="space-y-1 sm:space-y-2 lg:space-y-3">
                <li className="flex items-center gap-2 lg:gap-2 text-sm sm:text-base lg:text-base xl:text-lg">
                  ...
                  <span>Critical discourse leadership</span>
                </li>
                <li className="flex items-center gap-2 lg:gap-2 text-sm sm:text-base lg:text-base xl:text-lg">
                  <span className="text-green-500 text-lg lg:text-lg">✓</span>
                  <span>Cross-cultural text analysis</span>
                </li>
                <li className="flex items-center gap-2 lg:gap-2 text-sm sm:text-base lg:text-base xl:text-lg">
                  <span className="text-green-500 text-lg lg:text-lg">✓</span>
                  <span>Digital publication mastery</span>
                </li>
                <li className="flex items-center gap-2 lg:gap-2 text-sm sm:text-base lg:text-base xl:text-lg">
                  <span className="text-green-500 text-lg lg:text-lg">✓</span>
                  <span>Advanced literacy practices</span>
                </li>
              </ul>
            </div>
          </div>
