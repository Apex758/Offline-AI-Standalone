...
// Interactive advanced concepts component for Grade 5
function InteractiveAdvancedConcepts() {
  const [activeConcepts, setActiveConcepts] = useState<number[]>([])
  const concepts = [
    "PERSUASION",
    "ANALYSIS",
    "SYNTHESIS",
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setActiveConcepts((prev) => [...prev.slice(-3), randomIndex])
    }, 1400)
  ...
  return (
    <div className="flex flex-wrap justify-center gap-2 sm:gap-3 lg:gap-2 xl:gap-3 p-3 sm:p-6 lg:p-6 xl:p-8 bg-gradient-to-r from-purple-100 via-blue-100 to-emerald-100 rounded-xl sm:rounded-2xl lg:rounded-2xl">
      {concepts.map((concept, index) => (
        <div
          key={index}
          className={`px-3 py-2 sm:px-4 sm:py-3 lg:px-4 lg:py-3 xl:px-5 xl:py-3 rounded-full flex items-center justify-center text-xs sm:text-sm lg:text-sm xl:text-base font-bold cursor-pointer transition-all duration-300 ${
            activeConcepts.includes(index)
              ? "bg-gradient-to-r from-purple-500 to-emerald-500 text-white scale-110 sm:scale-125 lg:scale-130 shadow-lg"
              : "bg-white text-purple-600 hover:bg-purple-100 hover:scale-105 sm:hover:scale-110 lg:hover:scale-115"
          }`}
          onClick={() => setActiveConcepts((prev) => [...prev, index])}
        >
          {concept}
        </div>
      ))}
...
export default function Grade5LanguageArtsPage() {
  ...
  const strands = [
    {
      ...
      content:
        "Grade 5 students master sophisticated listening and speaking skills for academic and social success. They engage in complex discussions, debates, and presentations while developing critical thinking abilities. Students explore Caribbean dialects and Standard English, participate in formal speaking opportunities, and analyze persuasive techniques in media and advertisements with cultural sensitivity.",
      ...
      link: "/curriculum/grade5-subjects/language-arts/listening-speaking",
    },
    {
      id: 2,
      title: "Reading & Viewing",
      icon: Eye,
      description: "Critical analysis and advanced comprehension",
      content:
        "Students develop advanced reading strategies and critical thinking skills through engagement with diverse, culturally relevant texts. They analyze author's craft, explore complex themes, and make sophisticated connections across texts and cultures. Students master comprehension strategies, vocabulary development, and digital literacy while celebrating Caribbean literature and global perspectives.",
      ...
      link: "/curriculum/grade5-subjects/language-arts/reading-viewing",
    },
    {
      id: 3,
      title: "Writing & Representing",
      icon: PenTool,
      description: "Sophisticated composition and digital publishing",
      content:
        "Grade 5 students master the complete writing process through sophisticated composition across multiple genres. They develop advanced revision and editing skills, explore complex sentence structures, and create polished publications for authentic audiences. Students integrate technology, visual elements, and multimedia to enhance their written communication while maintaining cultural authenticity.",
      ...
      link: "/curriculum/grade5-subjects/language-arts/writing-representing",
    },
  ]
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-emerald-50">
      ...
      <div className="hidden sm:block fixed inset-0 pointer-events-none overflow-hidden">
        <FloatingElement delay={0}>
          <Globe className="absolute top-20 left-10 text-purple-400 w-4 h-4 sm:w-6 sm:h-6" />
        </FloatingElement>
        <FloatingElement delay={1}>
          <Award className="absolute top-40 right-20 text-emerald-400 w-6 h-6 sm:w-8 sm:h-8" />
        </FloatingElement>
        <FloatingElement delay={2}>
          <Lightbulb className="absolute bottom-40 left-20 text-blue-400 w-5 h-5 sm:w-7 sm:h-7" />
        </FloatingElement>
        <FloatingElement delay={0.5}>
          <Target className="absolute bottom-20 right-10 text-purple-400 w-4 h-4 sm:w-5 sm:h-5" />
        </FloatingElement>
      </div>
      ...
      <div className="container mx-auto px-4 lg:px-6 xl:px-8 py-4 sm:py-8 lg:py-10 xl:py-12 relative z-10 max-w-7xl">
        <Link to="/curriculum/grade5-subjects">
          ...
          >
            <ChevronLeft className="mr-1 sm:mr-2 h-3 w-3 sm:h-4 sm:w-4" /> Back to Grade 5 Subjects
          </Button>
        </Link>

        {/* Hero Section */}
        <div className="relative mb-8 sm:mb-12 lg:mb-16">
          <div className="bg-gradient-to-r from-purple-400 via-blue-400 to-emerald-400 p-4 sm:p-8 lg:p-10 xl:p-12 rounded-2xl sm:rounded-3xl shadow-xl sm:shadow-2xl transform hover:scale-105 transition-all duration-300">
            <div className="bg-white/90 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-7 xl:p-8">
              <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-4xl xl:text-5xl font-bold mb-3 sm:mb-6 lg:mb-6 text-center bg-clip-text text-transparent bg-gradient-to-r from-purple-600 via-blue-600 to-emerald-600 animate-pulse">
              ...
              <p className="text-sm sm:text-lg md:text-xl lg:text-xl xl:text-2xl text-gray-700 text-center max-w-4xl lg:max-w-5xl xl:max-w-6xl mx-auto leading-relaxed">
                The culmination of elementary language learning! Master sophisticated communication skills and prepare
                for middle school through advanced literacy practices and Caribbean cultural celebration! 📚✨
              </p>
            </div>
          </div>
        </div>
                ...
                <CardContent className="flex-grow p-4 sm:p-6 lg:p-6 flex flex-col">
                  ...
                  <div className="bg-yellow-100 p-3 sm:p-4 lg:p-4 rounded-lg mb-3 sm:mb-4 lg:mb-4 border-l-4 border-yellow-400">
                    <p className="text-xs sm:text-sm lg:text-sm text-yellow-800 font-medium">
                      💡 Grade 5 Mastery:{" "}
                      {strand.id === 1
                        ? "Lead complex debates and formal presentations!"
                        : strand.id === 2
                          ? "Analyze author's craft and cultural perspectives!"
                          : "Create multimedia publications for authentic audiences!"}
                    </p>
                  </div>
        ...
        {/* Interactive Advanced Concepts Section */}
        <div className="mb-8 sm:mb-12 lg:mb-16">
          <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-3xl xl:text-4xl font-bold mb-4 sm:mb-6 lg:mb-6 text-center text-purple-700 flex items-center justify-center gap-1 sm:gap-2 lg:gap-2">
            <span>🎓</span> Advanced Language Concepts! <span>🎓</span>
          </h2>
          <div className="max-w-7xl mx-auto">
            <InteractiveAdvancedConcepts />
          </div>
          <p className="text-center mt-3 sm:mt-4 lg:mt-4 text-gray-600 text-sm sm:text-base lg:text-base">
            Click on concepts to see them light up! Master advanced communication skills! ✨
          </p>
        </div>

        {/* About Section with Enhanced Design */}
        <div className="bg-gradient-to-br from-indigo-100 via-purple-100 to-blue-100 p-4 sm:p-6 md:p-8 lg:p-8 xl:p-10 rounded-2xl sm:rounded-3xl shadow-lg sm:shadow-xl border border-indigo-200 sm:border-2 mb-8 sm:mb-12 lg:mb-12 relative overflow-hidden max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 xl:gap-12">
            <div className="lg:col-span-1 xl:col-span-2">
              <p className="mb-3 sm:mb-4 lg:mb-4 text-gray-700 text-sm sm:text-lg lg:text-base xl:text-lg leading-relaxed">
                The Grade 5 curriculum serves as the capstone of elementary language education, preparing students for
                middle school success. Built on Essential Learning Outcomes, it emphasizes critical thinking and
                cultural appreciation! 🎯
              </p>
              <p className="text-gray-700 text-sm sm:text-lg lg:text-base xl:text-lg leading-relaxed">
                Students celebrate their Caribbean heritage while mastering both Home Languages and Standard English,
                preparing for academic success and lifelong learning! 🌴
              </p>
            </div>
            ...
            <div className="bg-white/70 p-4 sm:p-6 lg:p-6 xl:p-7 rounded-xl sm:rounded-2xl lg:col-span-1">
              ...
              <ul className="space-y-1 sm:space-y-2 lg:space-y-3">
                <li className="flex items-center gap-2 lg:gap-2 text-sm sm:text-base lg:text-base xl:text-lg">
                  ...
                  <span>Complex discussions & debates</span>
                </li>
                <li className="flex items-center gap-2 lg:gap-2 text-sm sm:text-base lg:text-base xl:text-lg">
                  <span className="text-green-500 text-lg lg:text-lg">✓</span>
                  <span>Cultural text analysis</span>
                </li>
                <li className="flex items-center gap-2 lg:gap-2 text-sm sm:text-base lg:text-base xl:text-lg">
                  <span className="text-green-500 text-lg lg:text-lg">✓</span>
                  <span>Multimedia composition</span>
                </li>
                <li className="flex items-center gap-2 lg:gap-2 text-sm sm:text-base lg:text-base xl:text-lg">
                  <span className="text-green-500 text-lg lg:text-lg">✓</span>
                  <span>Middle school readiness</span>
                </li>
              </ul>
            </div>
          </div>
