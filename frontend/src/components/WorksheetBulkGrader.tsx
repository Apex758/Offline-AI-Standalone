...
interface WorksheetBulkGraderProps {
...
const WorksheetBulkGrader: React.FC<WorksheetBulkGraderProps> = ({ worksheet, onClose, embedded = false }) => {
  ...
  const gradeableCount = worksheet.questions.filter(q =>
    q.type === 'multiple-choice' || q.type === 'true-false' || q.type === 'fill-blank' || q.type === 'word-bank'
  ).length;
  ...
  const handleGradeAll = async () => {
    ...
    try {
      ...
      form.append('worksheet_json', JSON.stringify(worksheet));
      studentFiles.forEach(f => form.append('student_files', f));

      const response = await axios.post(`${API_BASE}/api/bulk-grade-worksheet`, form, {
  ...
  const handleSave = async (resultIdx: number, result: FileGradeResult) => {
    ...
    try {
      ...
      await axios.post(`${API_BASE}/api/worksheet-grades`, {
        ...
        worksheet_title: worksheet.metadata.title,
    ...
    <div className={`flex flex-col ${embedded ? 'h-full' : 'h-full bg-theme-surface'}`}>
      {!embedded && (
        <div className="border-b border-theme px-6 py-4 flex items-center justify-between flex-shrink-0">
          <div>
            <h2 className="text-xl font-semibold text-theme-heading">Bulk Grade Worksheets</h2>
            <p className="text-sm text-theme-hint">
              {worksheet.metadata.subject} · {worksheet.metadata.title} · {gradeableCount} gradeable question{gradeableCount !== 1 ? 's' : ''}
            </p>
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="rounded-xl p-4 flex items-center gap-3 border" style={{ borderColor: `${accentColor}33`, backgroundColor: `${accentColor}08` }}>
          ...
          <div>
            <p className="text-sm font-medium text-theme-label">Answer key loaded from current worksheet</p>
            <p className="text-xs text-theme-muted">{worksheet.questions.length} total questions · {gradeableCount} auto-gradeable · {worksheet.questions.length - gradeableCount} open-ended (skipped)</p>
          </div>
        </div>
        ...
        <div>
          ...
          >
            ...
            <p className="text-sm font-medium text-theme-label">Drop student worksheet files here</p>
                  ...
                  {expandedRow === idx && !result.error && result.details.length > 0 && (
                    <div className="border-t border-theme divide-y divide-theme">
                      {result.details.map((d, di) => {
                        const correctLabel = String(d.correct_answer ?? '');
                        const studentLabel = d.student_answer === null ? 'No answer' : String(d.student_answer);
...
export default WorksheetBulkGrader;
