...
const PhaseContextBanner: React.FC<PhaseContextBannerProps> = ({ phase, onClear }) => {
  ...
  const label = `${phase.phase_label} (${new Date(phase.start_date).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })} \u2013 ${new Date(phase.end_date).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })})`;
  return (
    <div
      style={{
        display: 'flex',
