...
const WorksheetEditor: React.FC<WorksheetEditorProps> = ({ tabId, savedData, onDataChange }) => {
  ...
  return (
    <div className="h-full tab-content-bg grid grid-cols-2">
      {/* Left Panel - Editor (50%) */}
      <div className="flex flex-col border-r border-theme overflow-y-auto">
        <div className="border-b border-theme p-6 bg-gradient-to-r from-blue-50 dark:from-blue-900/30 to-indigo-50 dark:to-indigo-900/30">
          <h2 className="text-2xl font-bold text-theme-heading mb-4">Worksheet Editor</h2>
        ...
        <div className="p-6 max-h-[60vh] overflow-y-auto">
          <div className="space-y-4">
            {worksheetData.questions.map((question, index) => (
              <div key={question.id} className="border border-theme rounded-lg p-4 hover:border-blue-300 transition">
                {/* Question Header */}
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold text-theme-label">Question {index + 1}</span>
                    <select
                      value={question.type}
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      disabled={index === 0}
                      className="p-1 text-theme-hint hover:text-blue-600 disabled:opacity-30"
                      title="Move up"
                    >
                      ↑
                    </button>
                ...
                {(question.type === 'Multiple Choice' || question.type === 'True / False') && (
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <label className="block text-sm font-medium text-theme-label">Options</label>
          ...
          <button
            onClick={addQuestion}
            className="mt-4 w-full py-3 border-2 border-dashed border-theme-strong rounded-lg text-theme-muted hover:border-blue-400 hover:text-blue-600 transition flex items-center justify-center gap-2"
          >
      ...
      <div className="bg-theme-secondary border-l border-theme flex flex-col overflow-y-auto">
        <div className="p-4 border-b border-theme">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-theme-hint">Live preview of your worksheet</p>
            </div>
            
            {/* Student/Teacher Toggle */}
            <div className="flex items-center gap-2 bg-theme-surface border border-theme-strong rounded-lg p-1">
              <button
        ...
        <div className="flex-1 p-4">
          <div className="bg-theme-surface rounded-lg border border-theme h-full overflow-y-auto p-6">
            <div className="max-w-4xl mx-auto">
              ...
              <div className="space-y-6">
                {worksheetData.questions.map((question, index) => (
                  <div key={question.id} className="border-b border-theme pb-4">
                    <div className="flex items-start space-x-2">
                      <span className="font-semibold text-theme-label">{index + 1}.</span>
                      <div className="flex-1">
                        <p className="mb-2">{question.text || 'Question text not entered'}</p>

                        {question.type === 'Comprehension' && question.passage && (
                          <div className="bg-theme-tertiary p-4 rounded mb-4">
                        ...
                        {(question.type === 'Short Answer' || question.type === 'Fill in the Blank' || question.type === 'Calculations') && (
                          <div className="ml-4">
                            {viewMode === 'teacher' && question.correctAnswer ? (
                              ...
                              <div className="border-b border-theme-strong w-full h-6"></div>
                            )}
                          </div>
                        )}
                        ...
                        {question.type === 'Word Bank' && (
                          <div className="ml-4 mt-2">
                            <p className="text-sm text-theme-muted">Word Bank: {question.options?.join(', ') || 'No words specified'}</p>
                          </div>
                        )}
                      </div>
                    </div>
                ...
                {worksheetData.questions.length === 0 && (
                  <div className="text-center py-12 text-theme-hint">
